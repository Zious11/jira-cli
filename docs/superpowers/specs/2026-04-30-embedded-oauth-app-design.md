...
| Build pipeline injects malformed secret (non-UTF-8 after XOR) | `decode()` returns `Err`, so `build_embedded_app` collapses to `None` and the embedded app is treated as absent. Subsequent behavior is therefore the same as the "Embedded creds absent" cases above: use flag/env/keychain values if available; otherwise prompt interactively, or error under `--no-input`. The release smoke step (`OAuth app: embedded` row check) only verifies the constants are emitted, not that decode succeeded — true defense against a corrupt build would require extending the smoke to invoke a code path that actually decodes the secret (e.g., a future `oauth_login` end-to-end smoke). Decode failure is essentially unreachable for a valid XOR round-trip; this row exists for documentation completeness. |
