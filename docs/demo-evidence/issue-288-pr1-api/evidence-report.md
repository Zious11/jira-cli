...
| AC | Acceptance Criterion | Test(s) | File:line |
|----|----------------------|---------|-----------|
| AC-001 | `create_jsm_request` POSTs to `/rest/servicedeskapi/request`, deserializes `JsmRequestCreated` | `test_create_jsm_request_posts_to_servicedeskapi_and_returns_issue_key` | tests/jsm_request_api.rs:16 (assert line 56) |
| AC-002 | `list_request_types` paginates via `ServiceDeskPage` (`isLastPage`), accumulates all pages | `test_list_request_types_paginates_is_last_page` | tests/jsm_request_api.rs:65 (assert lines 147-150) |
| AC-003 | `search_query=Some(...)` forwarded as `searchQuery` param; `None` → param absent | `test_list_request_types_search_query_forwarded` (positive) + `test_list_request_types_search_query_absent_when_none` (negative) | tests/jsm_request_api.rs:158, :208 |
| AC-004 | `get_request_type_fields` GETs `.../requesttype/{rtId}/field`, returns `RequestTypeFieldsResponse` | `test_get_request_type_fields_returns_field_list` | tests/jsm_request_api.rs:259 (assert lines 306-313) |
| AC-005 | `RequestType` and `RequestTypeField` serde structs match Atlassian camelCase shape | `test_request_type_struct_round_trip` | tests/jsm_request_api.rs:322 (assert lines 337-360) |
| AC-006 | `JsmRequestCreated` exposes `issue_key` and `issue_id`; `issue_id` is `Option` | `test_jsm_request_created_extracts_issue_key` | tests/jsm_request_api.rs:368 (assert lines 374-383) |
