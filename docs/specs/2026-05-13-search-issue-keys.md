...
13. `test_search_issue_keys_repeated_cursor_abort_dedupes` — page 1 returns `["X-1"]` with `nextPageToken: "loop"`, page 2 returns `["X-1", "X-2"]` with the same `nextPageToken: "loop"` (simulating live-data drift mid-pagination); asserts `keys == ["X-1", "X-2"]` and `has_more == true`. Pins the **dedupe contract** under JRACLOUD-95368 abort: the incremental `seen_keys` HashSet (maintained outside the loop) eliminates the duplicate X-1 in first-occurrence order. Renamed and assertion flipped in #365.
...
- ~~**Possible duplicate keys on guard-abort under live-data drift (new finding from #361 research).** Because JRACLOUD-95368 is the typical cause, `search_issue_keys` *may* return duplicate keys in the abort path (live mutation can produce the same key on two pages before the cursor repeats). `search_issue_keys` does NOT dedupe today; the post-abort rustdoc and inline comment now warn callers explicitly and recommend `ORDER BY key ASC` as the upstream prevention. A separate follow-up issue (#365) tracks in-function dedupe.~~ **RESOLVED by #365 (2026-05-14).** Both `search_issue_keys` and `search_issues` now use an incremental `seen_keys: HashSet<String>` (maintained outside the pagination loop) to deduplicate on all exit paths — unique keys/issues are appended once in first-occurrence order without rescanning the accumulated Vec. All exit paths return a duplicate-free result vec. Spurious `--max` truncation errors and redundant bulk-edit calls due to drift-induced duplicates are eliminated. See `docs/specs/2026-05-14-search-issue-keys-dedupe.md` for full design rationale.
...
- `search_issues(jql, limit, extra_fields)` — signature unchanged. Semantics unchanged at spec-write time, but **PR #364 (issue #361 follow-up)** added one carve-out: `SearchResult.has_more` is now set to `true` on repeated-cursor guard abort (was silently `false`), matching the `KeySearchResult` contract this spec introduced. Three CLI readers (`cli/issue/list.rs`, `cli/board.rs`, `cli/queue.rs`) will display the existing "Showing N of M" truncation hint in that case where they previously silently reported a complete result. (`sprint.rs` calls `get_sprint_issues` (Agile API), not `search_issues`; `queue.rs` is the correct third reader.) No CLI flag, JSON shape, or error changes; see the Out-of-Scope bullet above for the full rationale.
...
- ~~**New follow-up (deferred):** `feat(search): dedupe keys on repeated-cursor guard abort` — under JRACLOUD-95368 live-data drift, the abort path may have already collected duplicate keys in earlier pages. Not blocking for the existing single caller (`handle_edit` truncates via `+1` lookahead) but worth addressing before a second caller appears.~~ **CLOSED by #365 (2026-05-14).** Incremental `seen_keys` HashSet dedupe implemented in both `search_issue_keys` and `search_issues`; see `docs/specs/2026-05-14-search-issue-keys-dedupe.md`.
