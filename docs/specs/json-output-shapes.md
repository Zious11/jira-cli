...
| `issue edit --dry-run` | `{"dryRun": true, "issues": [str], "plannedChanges": {...}}` (read-only preview; no mutation HTTP call) | Exactly three top-level keys. `plannedChanges` contains only explicitly-supplied-flag keys, with one derived exception: `descriptionAdf` (the real `adf::markdown_to_adf`/`text_to_adf` output, byte-identical to the live-path wire payload) is present iff a description input flag (`--description` or `--description-stdin`) is supplied — nested inside `plannedChanges`, never a fourth top-level key; `plannedChanges.description` remains the raw input string verbatim (BC-3.4.013/#398 unaffected). All other preview fields (`summary`, `priority`, `issueType`, `labels`, etc.) are intentionally simplified previews that do NOT match live-edit wire shapes. A `markdown_to_adf` `MAX_ADF_DEPTH` failure exits 64 with stdout EMPTY before any of this shape is emitted. S-692-1, BC-3.4.021, DEC-274. Verified at `src/cli/issue/edit.rs::handle_edit` dry-run block; `tests/issue_edit.rs`. |
...
| `issue attachment upload` | Success: JSON array of curated attachment objects (same shape as `attachment list` — see above). Cancel: `{"cancelled": true, "uploaded": false}` on stdout + `"Upload cancelled."` on stderr; exit 0. Dry-run: `{"dryRun": true, "wouldDelete": [{filename, id}], "wouldUpload": [{filename}]}` (camelCase keys; S-576-3). JSM `--public` dry-run: `wouldUpload` entries include `"visibility":"public"` (EC-3.9.020-7; S-576-5). | Success array shape: `[{author, contentUrl, created, filename, id, mimeType, size}]` (identical to list output — VP-576-004). Cancel shape emitted when user answers N to confirmation prompt; `--yes` bypasses. Dry-run shape emitted with `--dry-run`; `wouldDelete` is empty when `--replace-existing` is absent or no filename matches. JSM `--public` success: same curated array shape (step-2 returns curated list). Table output: 4-column table (Filename / Size / ID / Created). Implemented in `src/cli/issue/attachments.rs::handle_attachment_upload` (platform) + `handle_attachment_upload_jsm` (JSM). |
