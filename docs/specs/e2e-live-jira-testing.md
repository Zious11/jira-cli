...
- **JSM** (gated on `JR_E2E_JSM_PROJECT`; value `EJ`; skip cleanly when unset): thirteen-function
  ...
  the non-JSM guard) runs without it. Note: `test_e2e_comment_edit_visibility_merge_semantics`
  (S-577 MERGE-semantics test) is also gated on `JR_E2E_JSM_PROJECT` but is not part of the
  JSM feature family below; see CLAUDE.md § "jr issue comment edit --internal/--public". Covers
  queue list/view, requesttype list/fields, comment visibility, create round-trip, resolution
  enforcement, the non-JSM guard, and JSM attachment upload visibility.
...
| `JR_E2E_MENTION_ACCOUNT_ID` | variable (optional) | `5b10ac8d82e05b22cc7d4349` | S-cycle5-mention-resolution-wiring (issue #674): a CONTROLLED test account's real accountId (never a real person's) mentioned by the four `test_e2e_mention_*` round-trip tests (VP-674-014/015/016/017). Unset → all four clean-skip (early return) with a `[SKIP]` notice. Bracket-form `[~accountid:<id>]` mentions are used (not `@Name`) for determinism against a real, possibly multi-user Jira org — an `@Name` mention risks an ambiguous `ExactMultiple`/`Ambiguous` ONLY-MATCH failure the bracket form sidesteps entirely, since it needs only the accountId, not an exact-match-free display name. `test_e2e_mention_jsm_create_roundtrip` is additionally gated on `JR_E2E_JSM_PROJECT`. |
