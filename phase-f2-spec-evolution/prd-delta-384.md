...
1. ~~**`test_jsm_create_basic_auth_401_surfaces_api_token_hint`** (NEW — BC-3.8.014): Basic-auth fixture, generic 401 body → assert API-token-expiry hint surfaced.~~ **DELETED** (Copilot dedup — this test was a duplicate of `test_jsm_create_basic_auth_generic_401_surfaces_api_token_hint`; both used a Basic-auth fixture with a generic-expiry 401 body and the same assertions. The Basic-auth generic-expiry path is pinned by item #3 (`test_jsm_create_basic_auth_generic_401_surfaces_api_token_hint`), which remains as the BC-3.8.014 generic-expiry pin (AC-3 and AC-5 both describe this behavior and share that test).
