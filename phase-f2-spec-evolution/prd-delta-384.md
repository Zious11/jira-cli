...
> **[adversary-pass-4 F-02 note — SUPERSEDED by adversary-pass-9 C-01]** ~~`test_jsm_create_401_hint_contains_write_servicedesk_request` is NOT an acceptance test for BC-3.8.014. This test is being switched to a Bearer fixture and becomes a BC-3.8.015 (OAuth-path) pin; a Bearer-fixture test cannot be an acceptance test for BC-3.8.014 (Basic-auth). It is listed under BC-3.8.015 acceptance tests below. Cross-cutting update: the implementing story must update this test's fixture AND its ownership in the test file is implicitly BC-3.8.015 post-update.~~ The fixture switch from Basic to Bearer was unworkable: a Bearer + generic-expiry 401 routes through the refresh coordinator, which deterministically fails with raw anyhow (not a `JrError`) via the `JR_AUTH_HEADER` seam — the `write:servicedesk-request` hint is never injected. **The test stays on its Basic fixture and IS repurposed in place as a BC-3.8.014 acceptance pin** (assertions flipped to API-token-expiry hint). See Required Test Deliverables item #3 for the corrected plan.
...
> **[adversary-pass-5 F-02]** IMPORTANT: The OAuth *output* is genuinely unchanged — but the **existing regression pin `test_jsm_create_401_hint_contains_write_servicedesk_request` WILL FAIL after BC-3.8.014 lands**. Reason: that test currently uses a Basic-auth fixture (`JR_AUTH_HEADER=Basic dGVzdDp0ZXN0`) with a generic 401 body — precisely the pre-#384 bug scenario. After BC-3.8.014 lands, Basic+generic-401 MUST produce the API-token hint (not `write:servicedesk-request`), causing the existing assertion to fail. Migrating the test to a Bearer fixture is therefore a **mandatory hard prerequisite** of this delta — not a cosmetic refresh. The test cannot remain on a Basic-auth fixture post-merge.

**Acceptance tests**:
- `test_jsm_create_401_hint_contains_write_servicedesk_request` (MUST BE UPDATED — **mandatory prerequisite, not cosmetic**): switch fixture from `Basic dGVzdDp0ZXN0` to `Bearer test-oauth-token`; KEEP asserting `write:servicedesk-request`. This test IS H-NEW-JSM-RT-003 — the holdout and the test are the SAME artifact, both in `tests/issue_create_jsm.rs`. The "repurpose to Bearer" instruction and "the holdout revision" refer to one and the same change.
...
- H-NEW-JSM-RT-003 is realized AS `test_jsm_create_401_hint_contains_write_servicedesk_request` (see above — same artifact, not separate deliverable).
...
> **[adversary-pass-9 C-01 CRITICAL design correction]** Item #3 below has been corrected. The prior plan (switch `test_jsm_create_401_hint_contains_write_servicedesk_request` from Basic to Bearer + keep `write:servicedesk-request` assertion) was unworkable: a Bearer + generic-expiry 401 routes through the refresh coordinator, which fails with raw anyhow (not a `JrError`) via the `JR_AUTH_HEADER` seam, so the `write:servicedesk-request` hint is never injected. Item #3 is now a **repurpose-in-place** to BC-3.8.014 (fixture stays Basic; assertions flipped to API-token-expiry hint). Item #6 is added: the EXISTING `test_jsm_create_oauth_scope_mismatch_401_surfaces_write_servicedesk_request_hint` test (already green, must remain unmodified) is the BC-3.8.015 pin and the H-NEW-JSM-RT-003 re-binding.
...
3. **`test_jsm_create_401_hint_contains_write_servicedesk_request`** (MUST BE REPURPOSED IN PLACE — BC-3.8.014 ownership; fixture stays Basic): fixture STAYS `JR_AUTH_HEADER=Basic dGVzdDp0ZXN0` and generic-expiry 401 body — do NOT migrate to Bearer. F4 MUST flip assertions: (a) assert `contains` "expired or revoked", (b) assert `contains` `id.atlassian.com/manage-profile/security/api-tokens`, (c) assert `contains` `jr auth login`, (d) assert does NOT contain `write:servicedesk-request`. F4 may rename the test at its discretion. **WHY:** a Bearer + generic-expiry 401 routes through the refresh coordinator (client.rs:727+), which fails with raw anyhow (not a `JrError`) via the `JR_AUTH_HEADER` seam — the hint is never injected and the Bearer test would be non-deterministic. This is a BC-3.8.014 pin, NOT a BC-3.8.015 pin.
...
> **[adversary-pass-7 O-01, updated by adversary-pass-9 C-01]** The prior O-01 note (F4 must drop "scope-mismatch" wording from section banner when switching to Bearer) no longer applies — `test_jsm_create_401_hint_contains_write_servicedesk_request` stays Basic (no fixture switch). F4 MUST instead update the section banner and rustdoc to reflect the new BC-3.8.014 API-token-expiry framing (since the test is now a Basic-auth API-token pin, not an OAuth scope pin).
...
> **[adversary-pass-9 C-01 CRITICAL correction]** The prior F2 passes 1-8 had H-NEW-JSM-RT-003 realized as `test_jsm_create_401_hint_contains_write_servicedesk_request` with a planned fixture switch from Basic to Bearer + generic-expiry body. This plan was unworkable: a Bearer + generic-expiry 401 routes through the refresh coordinator (client.rs:727+), which deterministically fails with a raw anyhow error (not a `JrError`) via the `JR_AUTH_HEADER` seam — the `write:servicedesk-request` hint is never injected. H-NEW-JSM-RT-003 is NOW RE-BOUND to `test_jsm_create_oauth_scope_mismatch_401_surfaces_write_servicedesk_request_hint` (Bearer + scope-mismatch body — the ONLY deterministic OAuth→`JrError`→`write:servicedesk-request` path). `test_jsm_create_401_hint_contains_write_servicedesk_request` stays Basic and is repurposed to BC-3.8.014 (fixture stays, assertions flipped to API-token-expiry hint). Holdout count unchanged (55 total — re-bind, not add/remove).

> **[adversary-pass-5 F-01 — superseded by pass-9 C-01]** The prior F-01 note ("H-NEW-JSM-RT-003 is realized AS `test_jsm_create_401_hint_contains_write_servicedesk_request`") is SUPERSEDED by the pass-9 re-binding above.
...
> **[SUPERSEDED by Pass-6 F-01/F-02/F-03]** The holdout was regrounded to the real `HELP`/`id:99` fixture in Pass-6. The `HELPDESK`/`10001` bodies above are **historical and no longer authoritative** — they describe an intermediate fictional fixture that was corrected when the holdout was rewritten to faithfully describe the bound test `test_jsm_create_401_hint_contains_write_servicedesk_request`. The authoritative mock setup is in `holdout-scenarios.md` §H-NEW-JSM-RT-003 (project `HELP`, id `99`, `mount_project_meta_help`/`mount_service_desk_list`/`mount_request_type_list` helpers).
...
| F-02: BC-3.8.014 acceptance-test list in prd-delta included `test_jsm_create_401_hint_contains_write_servicedesk_request` (a Bearer-fixture test) under BC-3.8.014 (Basic-auth) — test-ownership contradiction | MEDIUM | Removed from BC-3.8.014 list; F-02 note added explaining it is repurposed to BC-3.8.015 ownership |
...
| Finding | Severity | Resolution |
|---------|----------|-----------|
| F-01: H-NEW-JSM-RT-003 test-file location contradiction — holdout body says `tests/issue_create_jsm.rs`; spec-changelog §Impact Assessment says `tests/issue_write_holdouts.rs`; ground truth is `tests/issue_create_jsm.rs` (H-NEW-JSM-RT-003 appears ONLY there, as annotations on `test_jsm_create_401_hint_contains_write_servicedesk_request`); the holdout and the test are the SAME artifact | HIGH | Spec-changelog Impact Assessment corrected; holdout section in prd-delta updated with F-01 note explicitly stating holdout IS realized as that test (same artifact, not two separate deliverables); BC-3.8.015 Trace note added; holdout-scenarios.md §H-NEW-JSM-RT-003 clarified |
| F-02: BC-3.8.015 "UNCHANGED" framing misleading — existing `test_jsm_create_401_hint_contains_write_servicedesk_request` uses Basic-auth fixture (`JR_AUTH_HEADER=Basic dGVzdDp0ZXN0` env line inside `async fn test_jsm_create_401_hint_contains_write_servicedesk_request`) with generic 401 body; post-#384, Basic+generic-401 MUST produce API-token hint per BC-3.8.014 → test WILL FAIL after BC-3.8.014 lands; Bearer migration is hard prerequisite, not cosmetic | HIGH | BC-3.8.015 section in prd-delta reworded: "OAuth *output* is unchanged, BUT existing regression pin WILL FAIL under BC-3.8.014 — migrating to Bearer fixture is a mandatory hard prerequisite." bc-3-issue-write.md §BC-3.8.015 test-instruction paragraph updated with same explicit warning |
...
| Finding | Severity | Resolution |
|---------|----------|-----------|
| F-01/F-02/F-03 (HIGH): H-NEW-JSM-RT-003 in holdout-scenarios.md accumulated fictional fixture detail across passes 2-4 (project `HELPDESK`/`id:10001`/`"Get IT Help"`/scope-mismatch body) that does not match the real bound test `test_jsm_create_401_hint_contains_write_servicedesk_request`. Title said "scope-mismatch" but the bound test uses a generic-expiry body routing via `NotAuthenticated`. Setup prescribed verbatim wiremock JSON for a non-existent fixture. Expected section included `stderr contains "Insufficient token scope"` which is FALSE for a generic-expiry body. Why-hidden text falsely stated the `InsufficientScope` branch is exercised. | HIGH | H-NEW-JSM-RT-003 fully rewritten to faithfully describe the real bound test: project `HELP`, `mount_project_meta_help`/`mount_service_desk_list`/`mount_request_type_list` helper fns described at behavioral abstraction, auth `Bearer test-oauth-token`, JSM POST 401 is generic-expiry body routing via `NotAuthenticated` arm; Expected removes `"Insufficient token scope"` line; title drops "scope-mismatch"; Why-hidden corrected to state `NotAuthenticated` arm; all accumulated blockquotes collapsed to single `[REVISED 2026-05-19 issue #384]` note. |
...
| O-01 (LOW): `test_jsm_create_401_hint_contains_write_servicedesk_request` section banner (line 1303) and rustdoc still say "scope-mismatch" but the test now uses a generic-expiry body routing via `NotAuthenticated`. | LOW | Note added to Required Test Deliverables: F4 implementation MUST update the section banner and rustdoc in `tests/issue_create_jsm.rs` to drop stale "scope-mismatch" wording when switching fixture to Bearer. |
