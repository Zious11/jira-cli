...
> **[adversary-pass-9 C-01 CRITICAL correction]** Prior instructions (items 1 and 4 below in the old draft) said "switch `test_jsm_create_401_hint_contains_write_servicedesk_request` to Bearer" — this was UNWORKABLE. See adversary-pass-9 §Corrections below. Corrected instructions:
...
3. `test_jsm_create_401_hint_contains_write_servicedesk_request` — REPURPOSED IN PLACE (BC-3.8.014 pin): fixture STAYS `JR_AUTH_HEADER=Basic dGVzdDp0ZXN0`; F4 MUST flip assertions from `write:servicedesk-request` to API-token-expiry hint; add negative assertion that `write:servicedesk-request` is ABSENT. F4 may rename at discretion. Do NOT switch to Bearer.
...
- **Affected stories:** 1 new story to implement (`is_oauth_auth` predicate + `API_TOKEN_EXPIRY_HINT` constant + gate both hint sites with rewrite logic + repurpose `test_jsm_create_401_hint_contains_write_servicedesk_request` to BC-3.8.014 pin with flipped assertions [fixture stays Basic, no Bearer migration] + add 3 new integration tests + `test_require_service_desk_oauth_401_surfaces_read_scope_hint` must use scope-mismatch body)
- **Affected tests:** `tests/issue_create_jsm.rs` — `test_jsm_create_401_hint_contains_write_servicedesk_request` stays Basic, assertions flipped to API-token-expiry hint (not `write:servicedesk-request`); 2 new Basic-auth tests added; 1 new `require_service_desk` Basic test; 1 new `require_service_desk` OAuth test with scope-mismatch body. `test_jsm_create_oauth_scope_mismatch_401_surfaces_write_servicedesk_request_hint` is the BC-3.8.015 pin AND H-NEW-JSM-RT-003 binding — remains green unmodified. (Adversary-pass-9 C-01 correction: prior statement that holdout is realized as `test_jsm_create_401_hint_contains_write_servicedesk_request` is SUPERSEDED.)
...
| M-01: Trace file paths valid | Confirmed: `tests/issue_create_jsm.rs` exists. NOTE (adversary-pass-5 F-01 correction): H-NEW-JSM-RT-003 is realized in `tests/issue_create_jsm.rs` — there is no separate `tests/issue_write_holdouts.rs` holding this holdout. The holdout annotations on `test_jsm_create_401_hint_contains_write_servicedesk_request` in `tests/issue_create_jsm.rs` are the holdout. |
...
| F-02: BC-3.8.014 acceptance-test list in prd-delta included `test_jsm_create_401_hint_contains_write_servicedesk_request` (a Bearer-fixture test) under BC-3.8.014 (Basic-auth) — this is a test-ownership contradiction; BC body already correctly scopes it to BC-3.8.015 | MEDIUM | Removed from BC-3.8.014 acceptance-test list; added explicit F-02 note explaining it is repurposed to BC-3.8.015 ownership (cross-cutting update) and is NOT a BC-3.8.014 acceptance test |
...
3. `test_jsm_create_401_hint_contains_write_servicedesk_request` (REPURPOSED IN PLACE — BC-3.8.014 pin; fixture STAYS Basic; F4 flips assertions to API-token-expiry hint; adds negative assertion that `write:servicedesk-request` is ABSENT; F4 may rename at discretion. Do NOT switch to Bearer — Bearer + generic-expiry routes through refresh coordinator and is not a valid pin.)
...
| Finding | Severity | Resolution |
|---------|----------|-----------|
| F-01: H-NEW-JSM-RT-003 test-file location contradiction — holdout body + changelog Impact Assessment cite `tests/issue_write_holdouts.rs`; ground truth is `tests/issue_create_jsm.rs` (the string `H-NEW-JSM-RT-003` appears ONLY there); the holdout and `test_jsm_create_401_hint_contains_write_servicedesk_request` are the SAME artifact | HIGH | Changelog Impact Assessment corrected: holdout is in `tests/issue_create_jsm.rs`, realized AS `test_jsm_create_401_hint_contains_write_servicedesk_request`. M-01 pass-2 table entry corrected. holdout-scenarios.md §H-NEW-JSM-RT-003 clarified. prd-delta-384.md BC-3.8.015 acceptance-test bullet and holdout sections clarified. BC-3.8.015 Trace in bc-3-issue-write.md updated to note test and holdout are same artifact, both in `tests/issue_create_jsm.rs`. |
| F-02: BC-3.8.015 "UNCHANGED" framing misleading — the existing `test_jsm_create_401_hint_contains_write_servicedesk_request` uses a Basic-auth fixture with generic 401 body (pre-#384 bug scenario); post-#384, Basic+generic-401 MUST produce the API-token hint per BC-3.8.014 → this test WILL FAIL after BC-3.8.014 lands; migration to Bearer fixture is a hard prerequisite, not cosmetic | HIGH | BC-3.8.015 section in prd-delta-384.md reworded explicitly: "OAuth *output* is unchanged, BUT the regression pin currently uses a Basic-auth fixture (the pre-#384 bug being fixed) and WILL FAIL under BC-3.8.014 — migrating to Bearer is a mandatory prerequisite." Same correction in bc-3-issue-write.md §BC-3.8.015 Trace/test-instruction block. |
...
| F-02: Hardcoded `tests/issue_create_jsm.rs:NNNN` line citations in F2 delta artifacts drift on every test insertion (e.g., the F4 fixture switch will shift line 1335 immediately) — conflicts with CLAUDE.md anti-drift convention | MEDIUM | Replaced EVERY `tests/issue_create_jsm.rs:NNNN` citation across `prd-delta-384.md`, `bc-3-issue-write.md`, `holdout-scenarios.md`, `spec-changelog.md` with symbol-relative anchors (`async fn` names and `// ─── section banner ───` references). `src/` line references retained (stable design anchors). De-pinned: `issue_create_jsm.rs:1523` → `async fn test_jsm_create_oauth_scope_mismatch_401_surfaces_write_servicedesk_request_hint` under `// ─── C-01 ───` banner; `issue_create_jsm.rs:1548` → `JR_AUTH_HEADER` env line inside that same `async fn`; `issue_create_jsm.rs:1335` → `JR_AUTH_HEADER` env line inside `async fn test_jsm_create_401_hint_contains_write_servicedesk_request`; `issue_create_jsm.rs:1309` → `async fn test_jsm_create_401_hint_contains_write_servicedesk_request`. |
