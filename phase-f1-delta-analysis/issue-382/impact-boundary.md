...
| `src/error.rs` unit test `insufficient_scope_exit_code` (line 131) | Constructs `JrError::InsufficientScope { message: "...".into() }` and asserts `exit_code() == 2` | **MODIFIED** — construction call adds `required_scope: None`; assertion (exit_code == 2) UNCHANGED |
...
|---|------|---------------|---------|------------------------|---------------|
| T-1 | `src/error.rs` | `insufficient_scope_display_includes_workarounds` | 170–186 (construction at **171**; assertion at **180**) | YES (line 180) | **YES** — construction call at line 171 adds `required_scope: None`; assertion at line 180 unchanged (None-fallback preserves `write:jira-work` literal in Display) |
| T-1b | `src/error.rs` | `insufficient_scope_exit_code` | 129–137 (construction at **131**) | No — only asserts `exit_code() == 2` | **YES** — needs `required_scope: None` when variant gains second field (option a); assertion `exit_code() == 2` UNCHANGED |
| T-2 | `tests/api_client.rs` | `test_401_scope_mismatch_returns_insufficient_scope` | 100–144 (assertion at **136**) | YES (line 136) | No — None-fallback at C-2 preserves `write:jira-work` literal in Display; assertion at line 136 still satisfied |
...
T-1 and T-1b require construction-call updates only. Both are construction sites in `src/error.rs` that need `required_scope: None` added when the variant signature widens under option (a): **2 test construction-call updates needed in `src/error.rs`** (lines 131 + 171). No assertion updates needed anywhere — T-1b's assertion pins only `exit_code() == 2` (unaffected), T-1's assertion at line 180 is preserved by the None-fallback which renders the `write:jira-work` literal in Display, and T-2 is likewise satisfied by the None-fallback at C-2.
...
- [REVISED 2026-05-19 per F1d adversary-pass-05 M-01 + L-03] — T-N label scheme harmonized to delta-analysis.md convention (T-1=display@171, T-1b=exit_code@131, T-2=api_client); Section 1 augmented with T-1b row.
  - M-01: Section 4 table relabeled: former T-1 (`insufficient_scope_exit_code` @131) → T-1b; former T-2 (`insufficient_scope_display_includes_workarounds` @171) → T-1; former T-3 (`tests/api_client.rs`) → T-2; former T-4–T-10 → T-3–T-9. Section 4 summary paragraph updated to reference T-1/T-1b instead of T-1/T-2/T-3.
  - L-03: Section 1 Impact Boundary Table augmented with new row for `src/error.rs` unit test `insufficient_scope_exit_code` (line 131): classified MODIFIED — construction call adds `required_scope: None`; assertion (exit_code == 2) UNCHANGED.
