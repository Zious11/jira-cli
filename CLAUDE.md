...
- **BC Trace fields must not contain numeric test counts.** The `Trace:` field in
  `.factory/specs/prd/bc-*.md` BC bodies should describe coverage qualitatively (file
  path + test category) rather than enumerate counts. Counts drift as tests are added;
  qualitative descriptions are stable. Enforced by `scripts/check-bc-no-numeric-test-counts.sh`
  in CI. Convention added by PG-365-1.
